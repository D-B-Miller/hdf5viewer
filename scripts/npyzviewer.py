...
class DataViewer:
    ''' DataViewer
        ...
        Wrapper GUI for a Matplotlib Figure showing the data given on creation. Called by NPZViewer when the user
    ...
    def scroll_data(self,*args):
        ...
        if args[0] == "moveto":
            ...
            self.axes.contourf(data[self.dataName][...,self.depth_index],cmap=self.curr_cmap)
            # update canvas
            self.fig_canvas.draw()
            # update title
    ...
    def update_line(self):
        ...
        data = np.load(self.filename)
        self.axes.plot(data[self.dataName][...,self.depth_index],cmap=self.curr_cmap)
        # update canvas
        self.fig_canvas.draw()
...
class NPZViewer:
    ''' NPZViewer
        ...
        Presents a graphical means for users to view and inspect the contents of a NPZ file.
    ...
    def open_file(self):
        ...
        self.curr_file = filedialog.askopenfilename(initialdir=os.path.dirname(self.curr_file),title="Select NPZ file to inspect",filetypes=[("NPZ files","*.npz")])
    ...
    def scan_file(self):
        ...
        if self.curr_file != "/":
            ...
            # load NPZ file
            data = np.load(self.curr_file,allow_pickle=True)
            # iterate over files
            for k,v in data.items():
                self.file_tree.insert('','end',text=k,values=(str(type(v)),str(v.shape),str(v.dtype)),open=True)
